...
var tianliGPT = {
  ...
  fetchTianliGPT: async function(content) {
    ...
    content = "我希望你充当一个摘要精简工具，帮助读者更容易地理解复杂的文章。当您的文章包含不必要的冗余信息时，使用清晰有力的语言来减少重复并强调内容的重点。上面为提示内容不必摘要, 以下为文章内容:" + content
    ...
    try {
        ...
        if ("[DONE]" == decodeURIComponent(event.data)) {
            StreamCatGPTFetchList.push("[DONE]");
            eventSource.close();
            return;
        }

        if (!JSON.parse(decodeURIComponent(event.data)).choices) {
          return ;
        }

        const content = JSON.parse(decodeURIComponent(event.data)).choices[0].delta.content
