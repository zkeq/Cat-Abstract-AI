...
var tianliGPT = {
  ...
  fetchTianliGPT: async function(content) {
    ...
    content = "我希望你充当一个摘要精简工具，帮助读者更容易地理解复杂的文章。当您的文章包含不必要的冗余信息时，使用清晰有力的语言来减少重复并强调内容的重点。可以通过以下几种方法来实现这一目标：首先，您应该了解文章摘要的作用：它是文章的简短概述，可以让读者快速理解文章的目的和关键内容。其次，您可以使用特定的工具（如摘要生成器）来自动生成文章摘要。或者，您可以使用自然语言处理技术（如NLP ）来设计算法以优化摘要生成过程。最后，要注意摘要的语言要简洁明了。将标题和内容精简为最重要的点，同时确保摘要涵盖了文章主要要点，以便读者可以快速了解文章。上面为提示内容不必摘要, 以下为文章内容:" + content
